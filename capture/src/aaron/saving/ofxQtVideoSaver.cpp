...
void ofxQtVideoSaver::startMovie(string movieName){
...
bail:    
	...
	printf("got to bail on startMovie() \n");
...
void ofxQtVideoSaver::setup( int width , int height){
...
//--------------------------------------------------------------
void ofxQtVideoSaver::setCodecType(string codeName ){
	...
	for (int i = 0; i < numCodecs; i++ ){
		...
		if (!strcmp(codeName.c_str(), typeName)){
			chosen = i;
			printf ("found the codec you wanted, number is (%i) \n", i);
...
/*  Save the current GWorld and set the offscreen GWorld as current.
	...
	float timeForQt = 1.0 / frameLengthInSecs;
...
string ofxQtVideoSaver::finalFileSavingSetting(){
    ...
    string aaronSettingCommand = "../../../data/./ffmpeg -i " + fileName + " -s 100x76 " + fileNameMinusExtension + "s.mov >/dev/null 2>&1";
    cout << aaronSettingCommand << endl;
	system( aaronSettingCommand.c_str() );
    
    return fileNameMinusExtension;
}

string ofxQtVideoSaver::movieNameFromTime(bool bAppendDirectory){
